...
-- ingresar datos 
insert into credenciales(usuario , password, rol_user ) 
...
insert into integrante(nombre, edad) values ('rafael',11),('pedrito',8);
insert into integrante(nombre, edad) values ('rafael',11);
select * from integrante;


select string.agg (substring('0123456789abcdefghijklmnopqrstuvwxyz_.!'), round(random()*40)::integer,1)," ")from  generate_series(1,$1);
$$ language sql;

select substring('0123456789abcdefghijklmnopqrstuvwxyz_.!', round(random()*length('0123456789abcdefghijklmnopqrstuvwxyz_.!'))::integer,1);
select string_agg(substring('0123456789abcdefghijklmnopqrstuvwxyz_.!', 
round(random()*length('0123456789abcdefghijklmnopqrstuvwxyz_.!'))::integer,1),'') as new_Password from  generate_series(1,15,1);
...
begin 
	...
	return query select string_agg(substring('0123456789abcdefghijklmnopqrstuvwxyz_.!', 
round(random()*length('0123456789abcdefghijklmnopqrstuvwxyz_.!'))::integer,1),'') as new_Password from  generate_series(0,long);
...
begin 
	...
	return (select string_agg(substring('0123456789abcdefghijklmnopqrstuvwxyz_.!', 
round(random()*length('0123456789abcdefghijklmnopqrstuvwxyz_.!'))::integer,1),'') as new_Password from  generate_series(0,long));
...
select generate_random_password1(3);


-- retornar un text pero de una variable 
create function generate_random_password2(long integer)
returns text as $$
declare ret text;
begin 

	ret=(select string_agg(substring('0123456789abcdefghijklmnopqrstuvwxyz_.!', 
round(random()*length('0123456789abcdefghijklmnopqrstuvwxyz_.!'))::integer,1),'') as new_Password from  generate_series(0,long));
	return ret;
end;
$$ language plpgsql;
...
select * from credenciales ;
update credenciales set password=sha512(generate_random_password1(12));
select sha512(password::bytea) from  credenciales ;
...
select * from credenciales where sha512('fgde.3wfu1syo')= password_sha and usuario='this58';


INSERT INTO usuarios(nombre,apellido,correo,ip,ubicacion,blog,usuario,seguidores,siguiendos) VALUES
